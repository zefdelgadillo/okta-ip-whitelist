...
URL = os.environ.get(
    'OKTA_URL')  # "https://s3.amazonaws.com/ok4ta-ip-ranges/ip_ranges.json"
...
def update_firewall(rule_name, ip_addresses):
    body = {'destinationRanges': ip_addresses}
    try:
        request = compute.firewalls().patch(project=PROJECT,
                                            firewall=f'okta-{rule_name}',
                                            body=body,
